...
import { getApiUrl } from '../config'; // 👈 IMPORT THIS!
...
const MyBookingsModal: React.FC<MyBookingsModalProps> = ({ isOpen, onClose, userId }) => {
  ...
  const fetchMyBookings = () => {
    // 🔥 UPDATED: Uses smart config to pick localhost or DDNS automatically
    fetch(`${getApiUrl()}/my-bookings/${activeUserId}`)
      .then(res => {
  ...
  const handleCancel = async (bookingId: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        fetchMyBookings(); // 🔄 Refresh list instantly
      } else {
        alert("Failed to cancel. It might already be deleted.");
      }
    } catch (error) {
